/**
 * tracker-md.mjs — markdown renderers (applications.md + win-rate.md).
 *
...
export function renderTracker(jobs, wr, queue, pipelineRows) {
  ...
  lines.push(`- Resume win-rate window: ${wr.windowDays} days`);
  lines.push(`- Applications in win-rate window: **${wr.total}**`);
  lines.push("");
  return lines.join("\n") + "\n";
}
...
export function renderWinRate(wr) {
  const now = new Date().toISOString();
  const lines = [];
  lines.push("# Resume Win-Rate Cache");
  lines.push("");
  lines.push(`> Rolling ${wr.windowDays}-day tally of \`job.yml.applied_with.resume\` across \`companies/*/jobs/*/job.yml\`.`);
  ...
  lines.push("## Tally");
  lines.push("");
  lines.push("| Engine | Wins | Apps | Win-Rate |");
  lines.push("|---|---|---|---|");
