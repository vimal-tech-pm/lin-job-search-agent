...
**Verdict rule (MANDATORY):** The verdict maps to the composite score per the table in `modes/_shared.md` (§ Scoring System). Use the EXACT verdict string for the score range. The verdict reflects ROLE FIT ONLY — do NOT fold Canada eligibility, location restrictions, or visa requirements into the verdict. Those are separate geo-gate fields. Example: a role scored 4.0 that is US-only remote gets verdict "Investable" and `canada_eligible: "no"` — the verdict says the role is a good fit; the geo gate prevents staging.
...
**Archetype:** {detected}
**Score:** {X/5}
**Verdict:** {verdict from _shared.md score→verdict table}
**Legitimacy:** {High Confidence | Proceed with Caution | Suspicious}
**PDF:** {path or pending}
**JD Snapshot:** jds/{###}-{company-slug}-{YYYY-MM-DD}.md
**Canada Eligible:** {yes|no|unknown} — {reason}
