...
This skill is copy-pasted from laptop to laptop via pendrive and personalized per candidate. Two layers exist and MUST NOT be confused:

**User Layer (NEVER auto-updated, personalization goes HERE):**
- `memory/coaching_state.md`, `memory/interview_tracker.md` — coaching continuity
- `source_files/*` — master resume, candidate background, archived docs
- `applications/active/*`, `applications/archive/*` — JDs, tailored resumes, exports
- `plans/*` — personal coaching workflow plans
...
- `resume-factory/*` (except user-customized `resume-factory/themes/` and `resume-factory/templates/`)
- `applications/README.md`, `memory/README.md` (structural templates only)

**THE RULE:** When personalizing the candidate — storybank, scores, notes, recruiter feedback, resume content, application tailoring — ALWAYS write to the User Layer (`memory/coaching_state.md`, `source_files/`, `applications/active/...`). NEVER edit `CLAUDE.md`, `AGENTS.md`, `references/*`, or any other System Layer file for candidate-specific content. This ensures system updates don't overwrite their customizations.

<!-- ============================================================
  VENDORED INTO LIN — UPDATE CHECK DISABLED.
  See ~/.hermes/profiles/lin/lin/PROVENANCE.md for the source commit.
  Updaters renamed to *.UPDATE-DISABLED. Do not re-enable.
  If you need a fix from upstream, port it manually and log in
  ~/.hermes/profiles/lin/lin/engines/forge/LIN-LOCAL-CHANGES.md.
============================================================ -->
...
1. **Session state**: Load and update `memory/coaching_state.md` if available. Everything else builds on continuity.
...
This skill maintains continuity across sessions using a persistent `memory/coaching_state.md` file.

### Session Start Protocol

At the beginning of every session:
1. Read `memory/coaching_state.md` if it exists.
...
1. Write the updated coaching state to `memory/coaching_state.md`.
2. Confirm: "Session state saved. I'll pick up where we left off next time."

### Mid-Session Save Protocol

Don't wait until the end to save. Write to `memory/coaching_state.md` after any major workflow completes (analyze, mock debrief, practice rounds, storybank changes) — not just at session close. If a long session is interrupted, the candidate shouldn't lose everything. When saving mid-session, don't announce it — just write the file silently and continue. Only confirm saves at session end.
...
After reading `memory/coaching_state.md`, check whether it contains all sections and columns defined in the current schema. Coaching state files created with earlier versions of the skill may be missing newer fields. If any are missing, migrate silently:
...
Write to `memory/coaching_state.md` whenever:
...
- apply creates or updates an application workspace under `applications/active/Company_Role/`, stores or references the JD source, creates a `source/` subfolder, runs a decode + tailored resume sequence, updates Interview Loops, updates the relevant JD Analysis section, updates Resume Optimization to reflect the new JD-targeted version, and updates `memory/interview_tracker.md`
- source builds or enriches candidate source material (save Source Material section to memory/coaching_state.md — last updated, last master sync, sync status, companies covered, pending sections, stale tailored variants). Auto-trigger master resume regeneration: update `source_files/<Name>_Master_Resume.md`, run export, update Resume Optimization section, flag stale tailored variants in Source Material section.
- linkedin produces profile audit (save LinkedIn Analysis section to memory/coaching_state.md — date, depth, overall score, dimension scores, top fixes pending, positioning gaps)
- resume produces resume audit (save Resume Optimization section to memory/coaching_state.md — date, depth, overall score, dimension scores, top fixes pending, JD-targeted status, cross-surface gaps)
- pitch produces a positioning statement (save Positioning Statement section to memory/coaching_state.md — date, depth, core statement, hook, key differentiator, earned secret anchor, target audience, variant status, consistency status)
- outreach produces outreach coaching (save Outreach Strategy section to memory/coaching_state.md — date, depth, positioning source, message types coached, targets contacted, channel strategy, follow-up status, LinkedIn profile flagged, key hooks identified)
- decode produces JD analysis (save JD Analysis section per JD to memory/coaching_state.md — date, depth, fit verdict, top competencies, frameable gaps, structural gaps, unverified assumptions, batch triage rank). Multiple JD Analysis sections can exist. Also update Interview Loops: if decode is for a company already in loops, add/update JD decode data; if new company, add lightweight entry with Status: Decoded.
- present produces presentation prep (save Presentation Prep section as top-level section in memory/coaching_state.md — include company name in header when company-specific — date, depth, framework, time target, content status, top predicted questions, key adjustment)
- salary produces comp strategy (save Comp Strategy section to memory/coaching_state.md — date, depth, target range, range basis, research completeness, stage coached, jurisdiction notes, scripts provided, key principle)
...
9. **Coaching meta-checks**. Every 3rd session (or when the candidate seems disengaged, defensive, or stuck), run a meta-check: "Is this feedback landing? Are we working on the right things? What's not clicking?" Build this into progress automatically, and trigger it ad-hoc when patterns suggest the coaching relationship needs recalibration. **To count sessions**: check the Session Log rows in `memory/coaching_state.md` at session start. If the row count is a multiple of 3, include a meta-check in that session regardless of which command is run. **After every meta-check**, record the candidate's response and any coaching adjustment to the Meta-Check Log in `memory/coaching_state.md`. Before running a meta-check, read the Meta-Check Log to reference previous feedback — build on past conversations rather than asking the same questions from scratch.
...
| `apply` | Start a new job application workflow: JD intake, decode, tailored resume, tracker/workspace updates |
...
- **`apply`**: Also read `references/commands/decode.md`, `references/commands/resume.md`, `references/differentiation.md`, and `references/storybank-guide.md`.
...
### Application Workspace Conventions

For application-specific work, keep each serious loop self-contained under:
- `applications/active/Company_Role/`

Inside that folder, maintain:
- `source/` for raw JD text, pasted recruiter notes, and company-specific source material
- exported `.pdf` and `.docx` resume files alongside the tailored `.md`
- the tailored resume markdown
- the JD decode
- any prep, follow-up, or interview-specific notes

When a loop closes, move its folder to `applications/archive/`.

### Resume Export Standard

All saved resume Markdown intended for export must follow the schema in `resume-factory/references/md-format-spec.md`.

Use:
- `resume-factory/references/md-format-spec.md` as the content contract for master and tailored resume `.md` files
- `resume-factory/themes/executive-clean.md` as the default formatting theme unless the user asks for another one
- `node resume-factory/scripts/build-resume.js <resume.md> executive-clean <output-base> --pdf` to generate `.docx` and `.pdf` outputs when export is needed (run `npm install --prefix resume-factory` once first; PDF rendering requires Playwright + Chromium, which the postinstall hook provisions)

Do not save resume Markdown in ad-hoc layouts like `# Name` plus freeform sections if the file is meant to become a PDF or DOCX later. The `.md` must stay buildable.

Never deliver a resume PDF that exceeds 2 pages. If the build produces >2 pages, follow the Page Overflow Recovery protocol in `resume-factory/SKILL.md` Step 6.
...
11. New application workflow / "help me apply to this role" / "tailor my resume to this JD" / "start a new application" intent -> `apply`
12. JD analysis / "decode this JD" / "is this role a good fit" / "should I apply" / "which of these roles should I pursue" / "compare these JDs" intent -> `decode`
13. Presentation prep / "I have a presentation round" / "help me structure my presentation" / "portfolio review prep" intent -> `present`
14. Comp questions / "what do I say about salary" / "recruiter asked about compensation" / "how do I handle the salary question" / "what should I put for expected salary" intent -> `salary`
...
21. Source material intent / "add to my source material" / "update my career details" / "I have new metrics" / "enrich my profile" -> `source`
22. Otherwise -> ask whether to run `kickoff` or `help`
...
| "Help me apply to this role" / "Tailor my resume to this JD" / "Start a new application" | `apply` |
...
| "I have new achievements to add" / "I have new metrics" | `source enrich` → (auto-sync master resume) |
