...
- Persistent session state via `memory/coaching_state.md` with mid-session saves
...
4 schema migration gap fixes ensuring older `memory/coaching_state.md` files fully upgrade:
...
Not replacing Claude Code, but a read-only dashboard that visualizes `memory/coaching_state.md`: score trends over time, storybank coverage heatmap, interview loop status, drill progression. Think of it as a `progress` command you can glance at without running anything. Could be a simple local web server that reads the markdown file.
...
### Full Web App
Backend, auth, database replacing `memory/coaching_state.md`, real UI for all commands. The skill files become the system prompt for an API-based product. The reference architecture is already modular enough to port — each command file maps to an API endpoint.
