...
**Session continuity** — A persistent `memory/coaching_state.md` file tracks your storybank, scores, patterns, drill progression, interview loops, interview intelligence, and calibration state across sessions. Pick up where you left off, weeks later. Saves are automatic.
...
**Application workflow** — A guided `apply` command that acts like application kickoff: asks for the JD one question at a time, runs a decode, creates a tailored resume, updates the tracker, and stores application artifacts in a dedicated workspace with output folders for PDF and Word versions. Resume exports now use the integrated `resume-factory` pipeline, so saved resume Markdown follows one strict schema and can compile deterministically into `.docx` and `.pdf`.
...
To verify the two entrypoint files stay in sync, run:

```bash
bash scripts/check-entrypoint-sync.sh
```

---

For both options, the coach will ask for your resume, target role, and timeline — then build your profile, assess your starting point, and give you a prioritized action plan. Everything saves automatically to `memory/coaching_state.md` so you pick up where you left off next session.
...
| `apply` | Start a new application workflow | JD intake + decode + tailored resume + tracker/workspace updates |
...
### 8.5) Start a full application workflow

```text
apply
```

Then provide the JD text, URL, or file. Get:

- application workspace creation under `applications/active/Company_Role/`
- a saved decode
- a tailored resume for that specific role
- output folders for PDF and DOCX resume exports
- tracker updates in `memory/interview_tracker.md`
...
├── AGENTS.md                           # Codex-oriented entry point
├── scripts/check-entrypoint-sync.sh    # Verifies CLAUDE.md and AGENTS.md match
├── README.md                           # This file
├── LICENSE                             # MIT License
├── memory/                             # User-specific session files (gitignored)
│   ├── README.md                       # Explains the folder and why only this file is tracked
│   ├── coaching_state.md               # `memory/coaching_state.md`, created on first kickoff
│   └── interview_tracker.md            # `memory/interview_tracker.md`, active application tracker
├── applications/                       # Per-application workspace
│   ├── active/
│   │   └── Company_Role/
│   │       ├── source/
│   │       ├── outputs/
│   │       │   ├── pdf/
│   │       │   └── docx/
│   │       ├── Company_Role_Decode.md
│   │       └── Candidate_Company_Role_Resume.md
│   └── archive/
├── source_files/                       # Raw resumes, LinkedIn exports, JD files, and source documents
└── references/
...
10. Run `apply` for serious applications — it creates the folder, decodes the JD, tailors the resume, and updates the tracker in one flow.
11. Run `decode` before applying when you only want a fit read or batch triage across multiple roles.
12. Run `salary` before your first recruiter call — the recruiter screen is the highest-leverage comp moment, not the offer negotiation.
13. Run `present` before a presentation round — structure your content and prepare for Q&A before you ever open PowerPoint.
