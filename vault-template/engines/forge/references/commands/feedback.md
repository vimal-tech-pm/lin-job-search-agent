...
- If the candidate provides career details, metrics, or achievements that belong to a specific company/role, also route to `source_files/candidate_source_material.md` — add to Verified Context and Metrics or Best Resume Material Candidates with `(status: pending)` for the relevant company. Update coaching state Source Material → Sync status: "stale".
