...
Update `memory/coaching_state.md` per the State Update Triggers in SKILL.md:
...
- Source Material enrichment: when debrief captures recalled metrics, context, or detail about past work not already in `source_files/candidate_source_material.md`, route to the relevant company section. Update coaching state Source Material → Sync status: "stale".
