...
- Company/role context (for audience calibration — or pulled from memory/coaching_state.md)
...
Pull from memory/coaching_state.md: Profile (target role, seniority), Interview Loops (company/role/format details), Prep Brief (if prep was run — evaluation criteria, culture, interviewer intel), Storybank (supporting stories to incorporate).
...
Save to memory/coaching_state.md as a top-level section (matching the schema in SKILL.md). Include the company name in the section header when company-specific:
