...
1. Read every story's full STAR text, earned secret, and deploy use-case from `memory/coaching_state.md`.
...
**Source Material Check**: When a story is added with new earned secrets, metrics, or details not already in `source_files/candidate_source_material.md`, enrich the relevant company's Storybank Candidates section (update status to `captured-as-S###`) and add any new metrics to Verified Context and Metrics. Update coaching state Source Material → Sync status: "stale".
...
**Source Material Check**: When a story is improved with new or updated earned secrets, metrics, or details not already in `source_files/candidate_source_material.md`, enrich the relevant company's Storybank Candidates section (confirm status `captured-as-S###`) and update Verified Context and Metrics if the improvement surfaces a better metric or outcome. Update coaching state Source Material → Sync status: "stale" if anything new was added.
