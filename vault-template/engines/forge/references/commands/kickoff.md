...
Save to memory/coaching_state.md Profile:
```
...
After resume analysis and reality check, check if `source_files/candidate_source_material.md` exists.

- **If it does NOT exist**: Build it from the resume and any additional documents provided (LinkedIn, career portfolio). Follow the `source build` workflow — create sections for each company with Canonical Role Snapshot, Verified Context and Metrics, Major Workstreams, Resume Material Candidates (status: pending/in-master), and Storybank Candidates (status: pending/captured-as-S###). Save to `source_files/candidate_source_material.md`.
- **If it DOES exist**: Run a quick freshness check — is the source material consistent with what the candidate just provided? Flag any discrepancies and update the relevant sections.

This ensures every kickoff produces a usable `candidate_source_material.md` that downstream commands (analyze, debrief, stories, feedback) can enrich automatically.

### Step 3: Initialize Coaching State

Before writing any memory files, ensure the folder exists with `mkdir -p memory`.

On first use, create both persistent memory files:
- `memory/coaching_state.md`
- `memory/interview_tracker.md`

Initialize `memory/interview_tracker.md` if it does not exist yet with these template headers:
...
Write the initial `memory/coaching_state.md` file (see SKILL.md Session State System for format) with:
...
This bootstrap step should succeed even in a fresh clone where `memory/` only exists conceptually because it is gitignored. Do not assume either memory file is already on disk.
...
3. **Update Profile in memory/coaching_state.md**: Target role, seniority band, career transition status (if newly triggered).
