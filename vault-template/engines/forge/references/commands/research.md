...
3. Assess fit against the candidate's profile (from `memory/coaching_state.md` if available, or from what they've told you).
...
When `research` is run for a company that already has a research entry in memory/coaching_state.md, check the date:
...
After research, save a lightweight entry to `memory/coaching_state.md` Interview Loops:
```
### [Company Name]
