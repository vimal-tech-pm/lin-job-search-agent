...
1. **Read `memory/coaching_state.md`** to understand where the candidate is in their coaching journey.
...
3. **Highlight the 2-3 most relevant commands right now** based on coaching state:
   ...
   - If the candidate wants to start a serious application from a JD and no application workspace exists yet for that role: highlight `apply`
...
| `apply` | Start a new application workflow — collect the JD, decode it, create a tailored resume, and update the tracker plus application workspace |
...
- Everything saves automatically to `memory/coaching_state.md` — pick up where you left off, even weeks later
