...
4. Include at least one question targeting a known story gap (from storybank gap analysis or `memory/coaching_state.md`) to test gap-handling under realistic conditions. When the candidate encounters this gap question, evaluate their response against the Gap-Handling Module patterns in `references/cross-cutting.md`. Note which pattern they used (or didn't) in the per-question debrief.
5. **Pull from saved concerns data.** If `concerns` was previously run for this company (check `memory/coaching_state.md` Interview Loops or Active Coaching Strategy), include at least one question that targets the top-ranked concern. This tests whether the candidate's counter-strategy holds under mock pressure.
