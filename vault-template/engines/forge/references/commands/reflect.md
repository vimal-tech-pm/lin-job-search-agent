...
2. **Pull the full data.** Review all of `memory/coaching_state.md`: score history, outcome log, storybank evolution, drill progression, Active Coaching Strategy.
...
[Note that memory/coaching_state.md is being preserved, not deleted — it's available if they resume]
...
- Do NOT delete `memory/coaching_state.md`. Mark it as archived with a date: add `Status: Archived [date] — [reason: accepted offer / paused search / etc.]` at the top.
