...
- The candidate's target companies and roles (from `memory/coaching_state.md`)
...
2. **Tailor questions to target roles.** Pull from `memory/coaching_state.md` — use the candidate's target companies, JDs, predicted questions from `prep`, and known weak competencies. Don't use generic questions if role-specific data exists.
