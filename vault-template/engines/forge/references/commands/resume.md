...
**Buildable Markdown Standard**: Any saved resume `.md` that should later become `.docx` or `.pdf` must follow `resume-factory/references/md-format-spec.md` exactly. See Step 8 for the export workflow. Use `resume-factory/themes/executive-clean.md` as the default theme and `node resume-factory/scripts/build-resume.js` for exports (the sole supported builder — DOCX via the `docx` library, PDF via Playwright + Chromium).
...
## Master Resume Convention

The master resume is the candidate's comprehensive, untailored resume — the single source of truth that all JD-targeted variants are derived from.

- **Location**: `source_files/<Name>_Master_Resume.md`
- **Created by**: `kickoff` (initial build) or `resume` in master optimization mode (subsequent improvements)
- **Format**: Must follow `resume-factory/references/md-format-spec.md` to remain buildable
- **Length**: The master resume has no page limit. It is a comprehensive document — include every role, bullet, and achievement worth having. The 2-page restriction applies only to exported/tailored variants, not the master. More material in the master means better tailoring options per JD.
- **Rule**: JD-targeted optimization always starts from the current master. Never tailor from a previously tailored variant — that compounds drift.

If no master resume exists when `resume` is invoked, check `source_files/` for any resume `.md`. If none found, prompt the candidate to provide their resume or run `kickoff` first.
...
- Target role context (from memory/coaching_state.md Profile, or ask)
...
Use the master resume from `source_files/` as the base document. If coaching state references a master resume path, use that. If no master exists in `source_files/`, ask the candidate to provide their resume or run `kickoff` first. Accept any format for initial intake — pasted text, copied from a doc, or described section by section. Be flexible.

### Step 2: Context Assembly

Pull from memory/coaching_state.md:
...
- **Master resume extraction**: Which bullets to keep/drop/promote for this specific application. Roles from the master resume are retained; relevance controls density, not inclusion.
- **Provide**: Complete targeted version (or the specific changes to make from the master resume)

### Experience Coverage Rule

For every JD-targeted resume, include every role from the master resume's Professional Experience section in the final tailored resume. Relevance controls density, not inclusion.

- Highly relevant roles: 3-6 bullets.
- Moderately relevant roles: 1-3 bullets.
- Low-relevance or older roles: 1 compact bullet.
- Internships or very short roles: include if present in the master resume, unless the candidate explicitly says to omit them.
- Do not remove a role solely to save space. Compress bullets, achievements, summary, and older-role detail first.
- If the 2-page limit is at risk, preserve the role header, company/date line, and one relevance-framed bullet.

### Step 8: Export (when a saved `.md` is produced)

When the optimization produces a saved resume `.md` (JD-targeted or master rewrite), offer export:

- Validate the `.md` against `resume-factory/references/md-format-spec.md`.
- Run `node resume-factory/scripts/build-resume.js <resume.md> executive-clean <output-base> --pdf` (first-time setup: `npm install --prefix resume-factory`, which also provisions Chromium for Playwright; `--pdf` exits 1 with an install hint if either is missing).
- Set `<output-base>` to the `.md` file path without its extension so the `.docx` and `.pdf` land alongside it.
- If this is part of an active application workspace, save the exports in the application folder. Otherwise, save them next to the source `.md`.
- After export, verify page count. If the resume exceeds 2 pages, apply the Page Overflow Recovery protocol from `resume-factory/SKILL.md` Step 6. Do not deliver a resume that exceeds the page target.

### Step 9: Cross-Surface Consistency Check (Deep only)
...
### Step 10: Challenge Protocol (Deep, Level 5 only)
...
After running `resume`, save to memory/coaching_state.md:
