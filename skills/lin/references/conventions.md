...
- **`HOME=$LIN_REAL_HOME`** for everything that touches Playwright/Chromium: `engines/pathfinder/generate-pdf.mjs`, `engines/forge/resume-factory/scripts/build-resume.js` renders, `scripts/cover-to-pdf.py`, `scripts/lin-verify-resumes.py` if it renders. The lin profile sandboxes `$HOME`; forgetting this is the #1 PDF failure cause.
